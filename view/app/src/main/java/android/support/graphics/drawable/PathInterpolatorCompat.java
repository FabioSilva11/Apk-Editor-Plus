...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private float[] f222mX;

    /* JADX INFO: renamed from: mY */
    private float[] f223mY;
    ...
    private void initPath(Path path) {
        ...
        this.f222mX = new float[iMin];
        this.f223mY = new float[iMin];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < iMin; i2++) {
            pathMeasure.getPosTan((i2 * length) / (iMin - 1), fArr, null);
            this.f222mX[i2] = fArr[0];
            this.f223mY[i2] = fArr[1];
        }
        if (Math.abs(this.f222mX[0]) > 1.0E-5d || Math.abs(this.f223mY[0]) > 1.0E-5d || Math.abs(this.f222mX[iMin - 1] - 1.0f) > 1.0E-5d || Math.abs(this.f223mY[iMin - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f222mX[0] + "," + this.f223mY[0] + " end:" + this.f222mX[iMin - 1] + "," + this.f223mY[iMin - 1]);
        }
        float f = 0.0f;
        int i3 = 0;
        while (i3 < iMin) {
            int i4 = i + 1;
            float f2 = this.f222mX[i];
            if (f2 < f) {
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f222mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f222mX[i2]) {
        ...
        float f2 = this.f222mX[length] - this.f222mX[i];
        if (f2 == 0.0f) {
            return this.f223mY[i];
        }
        float f3 = (f - this.f222mX[i]) / f2;
        float f4 = this.f223mY[i];
        return (f3 * (this.f223mY[length] - f4)) + f4;
    }
}
